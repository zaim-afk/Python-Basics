...
def attributes_check():
    ...
    if attributes['fun'] > 100:
        for line in ["Your fun", "rose", "above 100..."]:
            slow_text(line, 0.3)
            print()
        slow_text("That's physically impossible, therefore I'm lowering your fun.", 0.2)
        attributes['fun'] = 100
        print()
...
def skip_class():
    ...
    if teacher_catch in range(1,51):
        ...
        print()
        for line in ['You hear your name being called, so you turn and see.', 'Oh no! A teacher caught you!']:
            slow_text(line, 0.3)
            print()
        print()
...
def weekend():
    ...
    if weekend_choice == '1':
        ...
        if person_choice == 'friend':
            ...
            print()
            slow_text(f"{'He' if gender == 'male' else 'She' if gender == 'female' else 'They'} says sure.", 0.05)
            print()
...
def test():
    ...
    if attributes['energy'] <= 30:
        ...
        for line in ["You're in the middle of the test, when you hear a whisper.", f'"{name}...{name}"', "You turn to the source of the whisper."]:
            ...
            print()
        time.sleep(2)
        slow_text(f"You see a {'boy' if gender == 'male' else 'girl' if gender == 'female' else 'person'} making facial expressions and miming, and you can tell {'he' if gender == 'male' else 'she' if gender == 'female' else 'they'} is asking for the answer to a question ", 0.1)
        time.sleep(2)
        ...
        if cheat == 'y':
            ...
            slow_text(f"You look at the other student, look around and see some teachers, then look back at the other student with an annoyed expression. You choose not to help {'him' if gender == 'male' else 'her' if gender == 'female' else 'them'}.", 0.1)
            attributes['grades'] += 15
            attributes['social'] -= 5
    attributes_change()
    attributes_check()
...
def main():
    ...
    choose_name()
    if name == "Spam":
        print("""That's crazy. But did you know that Spam (stylized in all-caps) is a brand of processed canned pork and ham made by Hormel Foods Corporation, an American multinational food processing company. It was introduced in the United States in 1937 and gained popularity worldwide after its use during World War II.[1] As of 2003, Spam was sold in 41 countries, and trademarked in more than 100, on six continents.[2]
...
Spam has become part of popular culture, including a Monty Python sketch, which repeated the name many times, leading to its name being borrowed to describe unsolicited electronic messages, especially email.[8] It is occasionally celebrated at festivals such as Spamarama in Austin, Texas. Your welcome.""")
    time.sleep(0.5)
    cl_s()
    time.sleep(0.5)
    print(f"You are {name}. You are a Year 11 student at A.I.C. Kewdale.")
    slow_text("This game will have you play through Year 11, facing all the challenges and dealing with all the obstacles. During this game, you will be faced with options, and every choice you make affects your game. Beware, if your attributes go too low, you will lose.", 0.025)
    print()
    slow_text(f"These are the attributes you have to watch out for:\nGrades (currently at {attributes['grades']})\nEnergy (currently at {attributes['energy']})\nFun (currently at {attributes['fun']})\nSocial (currently at {attributes['social']}\nTrouble (currently at {attributes['trouble']}. This is the one attribute that you have to try to keep as low as possible)", 0.025)
    print()
    print()
    time.sleep(0.8)
    ...
    person, person_choice, principal = first_person()
    print()
    cl_s()
    time.sleep(1)
    if principal == True:
    ...
    if choice == '1':
        slow_text("You decide to go to class.", 0.05)
        print()
        attributes['grades'] += 20
        time.sleep(0.2)
        print(f"Your grades increased! It is now at {attributes['grades']}.")
    elif choice == '2':
        slow_text("You decide to skip class.", 0.05)
        attributes['grades'] -= 20
        print()
        time.sleep(0.2)
        print(f"Your grades decreased! It is now at {attributes['grades']}.")
    ...
    cl_s()
    time.sleep(1)
    slow_text("You continued going to school for the next few weeks, and luckily, no incidents happened.", 0.05)
    print()
    time.sleep(2)
    slow_text("Then, in Week 4, it was announced that Year 11 students would be performing their mid-term tests the next week.", 0.05)
    print()
    weekend()
    print()
    cl_s()
    ...
    new_term()
    cl_s()
    slow_text("Year 11's are told that they're going on an excursion this term. 🥳🥳", 0.1)
    print()
    excursion()
    cl_s()
    slow_text("It is announced that Semester exams would be taking place at the end of the term for all Year 11s.", 0.1)
    print()
    weekend()
    slow_text("It is Week 6, and you realise you aren't doing so well in English. You look at your timetable, and, coincidentally (or not), you have English.", 0.05)
    print()
    slow_text("Do you choose to go to English class, or skip it?", 0.05)
    print()
    choice = input("(1) Go to class\n(2) Skip class\n")
    while choice != '1' and choice != '2':
    ...
    weekend()
    cl_s()
    slow_text("It is now semester exam week.", 0.05)
    print()
    test()
    cl_s()
    term_report()
    ...
    new_term()
    cl_s()
    slow_text("Term 3 is pretty boring to be honest. You have mid-term tests in Week 5 and end-of-terms in Week 9. Oh well, at least you get more weekends this term ¯\\_(ツ)_/¯ (Hint: Use these weekends to improve (or unimprove) your attributes 😉)", 0.1)
    print()
    for no in range(1,5):
        print(f"Week {no} weekend: ")
        weekend()
        attributes_check()
        cl_s()
    print("It is now mid-term tests.")
    test()
    for no in range(5,9):
        print(f"Week {no} weekend: ")
        weekend()
        attributes_check()
        cl_s()
    print("It is now end-of term tests.")
    test()
    ...
    new_term()
    cl_s()
    slow_text("It is announced that Semester 2 exams would be taking place at the end of the term for all Year 11s.", 0.1)
    slow_text("And because it is Semester 2, the exams will be much harder.", 0.25)
    print("Your grades dropped 📉")
    attributes['grades'] -= 30
    print(f"They are now at {attributes['grades']}.")
    for no in range(1,5):
        print(f"Week {no} weekend: ")
        weekend()
        attributes_check()
    ...
    for no in range(5,9):
        ...
        attributes_check()
        cl_s()
    print("It is now semester exams.")
    test()
    for no in range(9,11):
        print(f"Week {no} weekend: ")
        weekend()
